...
and [`DR.Simple_UI.Catalogue/`](../src/DR.Simple_UI.Catalogue/CLAUDE.md).
...
1. Create `Components/Pages/<Name>.razor` in `src/DR.Simple_UI.Catalogue/`, with a `@page` route.
...
```bash
dotnet run --project src/DR.Simple_UI.Catalogue
```
...
Deployed to Railway from every push to `main`, built from `src/DR.Simple_UI.Catalogue/Dockerfile`
